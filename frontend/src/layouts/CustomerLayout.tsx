...
import { BottomNav, SideNav } from "@/components/layout/SideNav"
...
export function CustomerLayout() {
  ...
  return (
    <div className="flex h-full">
      <aside className="qc-aside hidden w-[220px] shrink-0 flex-col md:flex">
        ...
        <div className="border-t border-[var(--border)] px-4 py-3">
          <div className="mb-3 flex items-center justify-between gap-2">
            <span className="text-[10px] font-bold tracking-[0.12em] text-[var(--navy)] uppercase">
              Appearance
            </span>
            <ThemeToggle />
          </div>
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <div className="truncate text-xs font-medium text-[var(--text)]">
                {user ? `${user.firstName} ${user.lastName}` : "Arjun Kumar"}
              </div>
              <div className="truncate text-[10px] text-[var(--text-muted)]">
                {user?.email ?? "arjun@packtech.in"}
              </div>
            </div>
          </div>
        </div>
